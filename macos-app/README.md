...
- [ ] The dictionary panel is a clean, minimal card: **near-opaque ultra-thick material** (per the HIG, thicker materials for text-heavy content — the earlier translucent version read as muddy gray over a bright page), hairline system-separator border, the system window shadow only (no smeared dark rim at the edges), no mismatched lighter band behind the breadcrumb bar, plain numbered definitions (no filled circles), and borderless synonym chips. Check contrast in both light and dark mode over a bright PDF page.
